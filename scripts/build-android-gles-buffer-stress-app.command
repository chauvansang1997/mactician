...
readonly SOURCE_DIR="$PROJECT_DIR/artifacts/android-gles-buffer-stress-app"
...
readonly OUTPUT="${1:-$PROJECT_DIR/runtime/mactician-gles-buffer-stress.apk}"
readonly KEYSTORE="${TFT_GLES_STRESS_KEYSTORE:-$PROJECT_DIR/runtime/mactician-gles-probe-debug.keystore}"
readonly BUILD_DIR="$(mktemp -d -t mactician-gles-app)"
...
if ! command -v javac >/dev/null 2>&1 || ! command -v keytool >/dev/null 2>&1; then
    print -u2 "JDK javac and keytool are required to build the GLES stress app."
    exit 1
fi
...
javac \
    ...
    -d "$BUILD_DIR/classes" \
    "$SOURCE_DIR/src/dev/sergeinaumov/mactician/glesprobe/StressActivity.java"

typeset -a CLASS_FILES
CLASS_FILES=("$BUILD_DIR"/classes/**/*.class(N))
if (( ${#CLASS_FILES} == 0 )); then
    print -u2 "javac did not produce benchmark classes."
    exit 1
fi
"$BUILD_TOOLS/d8" \
    --lib "$ANDROID_JAR" \
