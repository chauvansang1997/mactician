...
| `run-tft-performance-max.command` | App Maximum FPS preset / source entrypoint | Reduce CPU/RHI and guest-host transport work while retaining full-resolution UI | 67% 3D scale, confirmed low-cost effects/LOD, four asynchronous OpenGL PSO compilers, 16 KiB ASG writes, and disabled ANGLE FBO-boundary submit; Trial 1-8 remained 34.1–35.1 FPS in two full transport confirmations | No |
